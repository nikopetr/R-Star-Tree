...
public class RunMultiple2DQueries {
    public static void main(String[] args){
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("pointRadiusQueryResults.csv"))) {
            ...
            // Radius Query File creation
            int j = 0;
            while(j < pointRadiusRStarTimes.size()){
                writer.write(String.format("%.5f", radiusOfCircles.get(j))+ "," + radiusQueryRecords.get(j) +"," +pointRadiusRStarTimes.get(j)+ "," + pointRadiusSeqScanTimes.get(j) + "\n");
                j++;
            }
        } catch (IOException e) {
            e.printStackTrace();
